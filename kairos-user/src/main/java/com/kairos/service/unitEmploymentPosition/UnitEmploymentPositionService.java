...
public class UnitEmploymentPositionService extends UserBaseService {
    ...
    public UnitEmploymentPositionQueryResult createUnitEmploymentPosition(Long id, String type, UnitEmploymentPositionDTO unitEmploymentPositionDTO) {
        ...
        if (!Optional.ofNullable(positionCode).isPresent()) {
            throw new DataNotFoundByIdException("positionCode Name does not exist in unit " + unitEmploymentPositionDTO.getPositionCodeId());
        }
            ...
            if (unitEmploymentPosition.getEndDateMillis() != null) {
                if (currentEndDateMillis != null) {
                    ...
                    if (previousInterval.overlaps(interval))
                        throw new ActionNotPermittedException("Already a unit employment positionCode is active with same expertise on this period(End date overlap with start Date)");

                } else {
                    if (new DateTime(currentEndDateMillis).isBefore(new DateTime(unitEmploymentPosition.getEndDateMillis()))) {
                        throw new ActionNotPermittedException("Already a unit employment positionCode is active with same expertise on this period(End date overlap with start Date)." + new DateTime(currentEndDateMillis).toDate() + " --> " + new DateTime(unitEmploymentPosition.getEndDateMillis()).toDate());
                    }
                }
            } else {
                ...
                if (currentEndDateMillis != null) {
                    if (new DateTime(currentEndDateMillis).isAfter(new DateTime(unitEmploymentPosition.getStartDateMillis()))) {
                        throw new ActionNotPermittedException("Already a unit employment positionCode is active with same expertise on this period(End date overlap with start Date)" + new DateTime(currentEndDateMillis).toDate() + " --> " + new DateTime(unitEmploymentPosition.getStartDateMillis()).toDate());
                    }
                } else {
                    throw new ActionNotPermittedException("Already a unit employment positionCode is active with same expertise on this period.");
                }
            }
        });
        return true;
    ...
    public PositionWrapper updateUnitEmploymentPosition(long unitEmploymentPositionId, UnitEmploymentPositionDTO unitEmploymentPositionDTO) {
        ...
        if (!Optional.ofNullable(oldUnitEmploymentPosition).isPresent()) {
            throw new DataNotFoundByIdException("Invalid positionId id " + unitEmploymentPositionId + " while updating the positionCode");
        }
