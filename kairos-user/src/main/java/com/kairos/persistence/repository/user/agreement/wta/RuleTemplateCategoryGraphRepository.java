...
public interface RuleTemplateCategoryGraphRepository extends Neo4jBaseRepository<RuleTemplateCategory, Long> {
    ...
    @Query("match(o:Organization)-[:" + BELONGS_TO + "]-(c:Country{isEnabled:true})-[r:HAS_RULE_TEMPLATE_CATEGORY]-(l:RuleTemplateCategory{deleted:false,ruleTemplateCategoryType:'WTA'}) where Id(o)={0} \n" +
            "OPTIONAL MATCH (l)-[:HAS_TAG]-(t:Tag{deleted:false,masterDataType:'RULE_TEMPLATE_CATEGORY'})<-[:COUNTRY_HAS_TAG]-(c) WHERE t.masterDataType='RULE_TEMPLATE_CATEGORY' \n" +
            "RETURN id(l) as id, l.name as name, l.description as description,CASE when t IS NULL THEN [] ELSE collect({id:id(t),name:t.name,countryTag:t.countryTag})  END as tags")
