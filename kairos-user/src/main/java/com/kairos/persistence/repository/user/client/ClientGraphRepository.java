...
public interface ClientGraphRepository extends GraphRepository<Client>{
    ...
    @Query("Match (client:Client{citizenDead:false})-[:"+GET_SERVICE_FROM+"]->(organization:Organization) where id(organization)={0} with client\n" +
            ...
            "return id(client) as id,client.firstName+\" \" +client.lastName as name,client.gender as gender,client.profilePic as profilePic,client.age as age, ca.houseNumber+\" \" +ca.street1 as address, collect({id:staffId,type:case when r is null then 'NONE' else r.type end}) as staff")
    ...
    @Query("Match (client:Client)-[r:"+CLIENT_CONTACT_PERSON_RELATION_TYPE+"]->(clientContactPerson:ClientContactPerson) where id(client) in {0} AND id(clientContactPerson)={2} delete  r")
