...
public interface TagGraphRepository extends GraphRepository<Tag> {
    ...
    @Query("Match (country:Country)-[r:"+COUNTRY_HAS_TAG+"]->(tag:Tag {countryTag:true})\n" +
            ...
            "return tag")
    List<Tag> getListOfCountryTags(Long countryId , boolean deleted, String searchTextRegex);
    ...
    // DONE
    @Query("MATCH (c:Country) WHERE id(c)={0} CREATE (c)-[:"+COUNTRY_HAS_TAG+"]->(tag:Tag{name:{1}, countryTag:true, masterDataType:{2}, deleted:false}) return tag")
    Tag createCountryTag(Long countryId, String tagName, String masterDataType);
