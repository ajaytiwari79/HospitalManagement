...
public interface StaffGraphRepository extends GraphRepository<Staff> {
    ...
    @Query("MATCH (employment:UnitEmployment) where id(employment) = {0} SET employment.startDate={0},employment.endDate={1},employment.salary={2} RETURN employment")
    UnitEmployment updateEmployment(long id, long startDate, long endDate, Float salary);

    @Query("MATCH (partialLeave:PartialLeave) where id(partialLeave) = {0} SET partialLeave.startDate={1},partialLeave.endDate={2},partialLeave.amount={3},partialLeave.employmentId={4}, partialLeave.note={5} RETURN partialLeave")
    PartialLeave updatePartialLeave(long id, long startDate, long endDate, float amount, String employmentId, String note);
    ...
    @Query("Match (organization:Organization)-[:HAS_EMPLOYMENTS]->(employment:Employment)-[:BELONGS_TO]->(staff:Staff)-[:HAS_CONTACT_DETAIL]->(contactDetail:ContactDetail{privateEmail:{1}}) where id(organization)={0} return staff")
    Staff getByEmail(long organizationId,String email);

    @Query("Match (unit:Organization),(staff:Staff) where id(unit)={0} AND id(staff)={1}\n" +
            "Match (unit)-[r:"+ORGANISATION_HAS_SKILL+"{isEnabled:true}]->(skill:Skill)<-[staffSkillRel:"+STAFF_HAS_SKILLS+"{isEnabled:true}]-(staff)\n" +
    ...
    @Query("Match (staff:Staff),(skill:Skill) where id (staff)={0} AND id(skill)={1} create (staff)-[r:"+STAFF_HAS_SKILLS+"{creationDate:{2},lastModificationDate:{3},skillLevel:{4},isEnabled:true,startDate:0,endDate:0}]->(skill) return skill")
    void addSkillInStaff(long staffId, long skillId, long creationDate, long lastModificationDate, Skill.SkillLevel skillLevel);

    @Query("Match (staff:Staff),(skill:Skill) where id (staff)={0} AND id(skill) = {1} Match (staff)-[r:"+STAFF_HAS_SKILLS+"]->(skill) set r.creationDate={2},r.lastModificationDate={3},r.isEnabled=true return skill")
    void updateSkillOfStaff(long unitId, long skillId, long creationDate, long lastModificationDate);
