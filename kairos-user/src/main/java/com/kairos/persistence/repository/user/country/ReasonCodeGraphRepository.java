...
public interface ReasonCodeGraphRepository extends Neo4jBaseRepository<ReasonCode,Long> {
    ...
    @Query("MATCH (country:Country)-[:"+BELONGS_TO+"]-(reasonCode:ReasonCode{deleted:false}) where id(country)={0} return id(reasonCode) as id, reasonCode.name as name," +
            "reasonCode.code as code, reasonCode.description as description,reasonCode.reasonCodeType as reasonCodeType ORDER BY id")
    List<ReasonCodeResponseDTO> findReasonCodesByCountry(long countryId);
