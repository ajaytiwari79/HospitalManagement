...
public interface AccessPageRepository extends GraphRepository<AccessPage> {
    ...
    @Query("Match (accessGroup:AccessGroup),(accessPermission:AccessPermission) where id(accessPermission)={0} AND id(accessGroup)={1}\n" +
            "Match (accessGroup)-[r:"+HAS_ACCESS_OF_TABS+"]->(accessPage:AccessPage) with accessPage,r,accessPermission\n" +
    ...
    @Query("MATCH (n:Organization)-[:HAS_EMPLOYMENTS]->(emp:Employment)-[:BELONGS_TO]->(staff:Staff)-[:BELONGS_TO]->(user:User) where id(n)={0} AND  id(staff)={2}\n" +
            ...
            "Match (unitEmp)-[:HAS_ACCESS_PERMISSION]->(ap:AccessPermission)-[:HAS_ACCESS_GROUP]->(g:AccessGroup) where id(g)={3} with ap,g\n" +
            ...
            "optional Match (ap)-[r2:HAS_ACCESS_PAGE_PERMISSION]->(child) with ps,parent,r2\n" +
    ...
    @Query("MATCH (org:Organization) where id(org)={0} with org\n" +
            ...
            "Match (unitEmp)-[:"+HAS_ACCESS_PERMISSION+"]->(ap:AccessPermission) with ap,org\n" +
            "Match (ap)-[r:"+HAS_ACCESS_PAGE_PERMISSION+"]->(accessPage:AccessPage{isModule:true})\n" +
    ...
    @Query("MATCH (org:Organization) where id(org)={0} with org\n" +
            ...
            "Match (accessPermission)-[modulePermission:"+HAS_ACCESS_PAGE_PERMISSION+"]->(module:AccessPage{isModule:true}) with module,accessPermission,modulePermission\n" +
            "Match (module)-[:"+SUB_PAGE+"*]->(subPage:AccessPage)<-[subPagePermission:"+HAS_ACCESS_PAGE_PERMISSION+"]-(accessPermission) with module,subPage,modulePermission,subPagePermission\n" +
