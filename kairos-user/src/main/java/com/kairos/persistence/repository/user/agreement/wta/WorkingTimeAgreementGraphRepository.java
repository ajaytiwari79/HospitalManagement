...
public interface WorkingTimeAgreementGraphRepository extends GraphRepository<WorkingTimeAgreement> {
    ...
    @Query("match(wta:WorkingTimeAgreement{isEnabled:true})-[:" + BELONGS_TO_ORG_SUB_TYPE + "]->(o:OrganizationType) where id(o)={0} with wta,o\n" +
            "match(wta)-[:" + HAS_EXPERTISE_IN + "]->(expertise:Expertise) with wta,o,expertise\n" +
            "optional match(wta)-[:" + HAS_RULE_TEMPLATE + "]->(ruleTemp:WTABaseRuleTemplate)<-[:" + HAS_RULE_TEMPLATES + "]-(ruleTemplateCategory:RuleTemplateCategory) with wta,o,ruleTemp,expertise,ruleTemplateCategory,\n" +
