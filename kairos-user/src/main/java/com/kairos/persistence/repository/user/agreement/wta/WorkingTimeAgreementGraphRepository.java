...
public interface WorkingTimeAgreementGraphRepository  extends GraphRepository<WorkingTimeAgreement> {
    ...
    @Query("match(wta:WorkingTimeAgreement{isEnabled:true})-[:BELONGS_TO]->(c:Country) where id(c)=53\n" +
            ...
            "with wta,expertise,orgType,orgSubType, collect ({name:ruleTemp.name,id:Id(ruleTemp),minimumRest:ruleTemp.minimumRest,templateType:ruleTemp.templateType,nightsWorked:ruleTemp.nightsWorked,\n" +
            "description:ruleTemp.description,checkAgainstTimeRules:ruleTemp.checkAgainstTimeRules,ruleTemplateCategory:ruleTemplateCategory}) as ruleTemplates\n" +
