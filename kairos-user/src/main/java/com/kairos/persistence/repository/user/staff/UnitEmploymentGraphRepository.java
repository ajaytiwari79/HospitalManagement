 ...
 */
@Repository
public interface UnitEmploymentGraphRepository extends Neo4jBaseRepository<UnitPermission, Long> {
    ...
    UnitPermission getUnitEmployment(long parentOrganizationId, long staffId, long childOrganizationId);

    @Query("Match (organization:Organization),(staff:Staff) where id(organization)={0} AND id(staff)={1} with organization,staff\n" +
            "Match (organization)-[:" + HAS_EMPLOYMENTS + "]->(employment:Employment)-[:" + BELONGS_TO + "]->(staff) with employment\n" +
            "Match (employment)-[:" + HAS_UNIT_PERMISSIONS + "]->(unitPermission:UnitPermission{employmentStatus:{3}}) with unitPermission\n" +
            "match (unitPermission)-[r:" + APPLICABLE_IN_UNIT + "]->(unit:Organization) where id(unit)={2} return unitPermission")
    UnitPermission getUnitEmployment(long parentOrganizationId, long staffId, long childOrganizationId, EmploymentStatus employmentStatus);
    ...
    UnitPermission checkUnitEmploymentOfStaff(Long parentOrganizationId, Long organizationId, Long staffId);
    ...
    @Query("Match (organization:Organization),(staff:Staff) where id(organization)={0} AND id(staff)={1} with organization,staff \n" +
            ...
            "return  unitPermission")
    UnitPermission checkUnitEmploymentOfStaff(Long parentOrganizationId, Long staffId);
}
