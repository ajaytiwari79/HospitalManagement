...
public interface UnitEmploymentGraphRepository extends Neo4jBaseRepository<UnitEmployment, Long> {
    ...
    @Query("Match (organization:Organization),(staff:Staff) where id(organization)={0} AND id(staff)={2} with organization,staff \n" +
            ...
            "Match (employment)-[:HAS_UNIT_EMPLOYMENTS]->(unitEmployment:UnitEmployment)\n" +
            "match(unitEmployment)-[:PROVIDED_BY]-(co:Organization) where id(co)={1}\n" +
            "return  unitEmployment")
    UnitEmployment checkUnitEmploymentOfStaff(Long parentOrganizationId, Long organizationId, Long staffId);

    // for parent organization
    @Query("Match (organization:Organization),(staff:Staff) where id(organization)={0} AND id(staff)={1} with organization,staff \n" +
            "Match (organization)-[:HAS_EMPLOYMENTS]->(employment:Employment)-[:BELONGS_TO]->(staff)  \n" +
            "Match (employment)-[:HAS_UNIT_EMPLOYMENTS]->(unitEmployment:UnitEmployment)-[:PROVIDED_BY]-(organization)\n" +
            "return  unitEmployment")
    UnitEmployment checkUnitEmploymentOfStaff(Long parentOrganizationId, Long staffId);
}
