...
public interface TeamGraphRepository extends GraphRepository<Team>{
    ...
    @Query("Match (team:Team) where id(team)={0} with team\n" +
            ...
            "Match (organization)-[:HAS_EMPLOYMENTS]->(employment:Employment) with employment,organization,team\n" +
    ...
    @Query("Match (team:Team) where id(team)={0}\n" +
            "Match (team)<-[:"+HAS_TEAM+"]-(group:Group)<-[:"+HAS_GROUP+"]-(organization:Organization) with organization\n" +
            "Match (organization)-[r:"+ORGANISATION_HAS_SKILL+"]->(skill) with skill,r\n" +
            "MATCH (skill{isEnabled:true})-[:"+HAS_CATEGORY+"]->(skillCategory:SkillCategory{isEnabled:true}) with {id:id(skillCategory),name:skillCategory.name,children:collect({skillId:id(skill),name:skill.name,description:skill.description,visitourId:r.visitourId,isEdited:true})} as availableSkills\n" +
            "return {availableSkills:collect(availableSkills)} as data\n" +
            "UNION\n" +
            "Match (team:Team) where id(team)={0}\n" +
            "match (team)-[r:"+TEAM_HAS_SKILLS+"{isEnabled:true}]->(skill) with skill,r\n" +
            "MATCH (skill{isEnabled:true})-[:"+HAS_CATEGORY+"]->(skillCategory:SkillCategory{isEnabled:true}) with {id:id(skillCategory),name:skillCategory.name,children:collect({skillId:id(skill),name:skill.name,description:skill.description,visitourId:r.visitourId,isEdited:true})} as selectedSkills\n" +
            "return {selectedSkills:collect(selectedSkills)} as data")
    List<Map<String,Object>> getSkillsOfTeam(long teamId);

    @Query("Match (team:Team)-[r:TEAM_HAS_SKILLS]->(s:Skill) where id(team)={0} AND id(s)={1} return count(r) as countOfRel")
    int countOfSkillInTeam(long teamId, long skillId);

    @Query("Match (team:Team),(skill:Skill) where id (team)={0} AND id(skill) IN {1} create (team)-[r:"+TEAM_HAS_SKILLS+"{creationDate:{2},lastModificationDate:{3},isEnabled:true}]->(skill) return skill")
    void addSkillInTeam(long teamId, List<Long> skillId, long creationDate, long lastModificationDate);


    @Query("Match (team:Team),(skill:Skill) where id (team)={0} AND id(skill) IN {1} Match (team)-[r:"+TEAM_HAS_SKILLS+"]->(skill) set r.lastModificationDate={2},r.isEnabled={3} return skill")
    void updateSkillInTeam(long teamId, List<Long> skillId, long lastModificationDate,boolean isEnabled);
