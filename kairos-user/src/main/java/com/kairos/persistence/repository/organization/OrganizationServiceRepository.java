...
public interface OrganizationServiceRepository extends GraphRepository<OrganizationService>{
    ...
    @Query("MATCH (c:Country),(n:OrganizationService{isEnabled:true}) where id(c)={0} match (c)-[:HAS_ORGANIZATION_SERVICES]->(n) OPTIONAL MATCH (n)-[:ORGANIZATION_SUB_SERVICE]->(s:OrganizationService{isEnabled:true}) RETURN {children: case when s is NULL then [] else collect({id:id(s),name:s.name,description:s.description}) END,id:id(n),name:n.name,description:n.description} as result")
    ...
    @Query("MATCH (o:Organization)-[:PROVIDE_SERVICE]->(os:OrganizationService{isEnabled:true}) where id(o)={0}  return id(os) ")
