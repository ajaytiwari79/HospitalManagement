...
public class ActivityLineIntervalPillarMove extends AbstractMove<ShiftRequestPhasePlanningSolution> {
    ...
    protected void doMoveOnGenuineVariables(ScoreDirector<ShiftRequestPhasePlanningSolution> scoreDirector) {
        ...
        log.debug("*P1*********************STARTS********************");
        for(ActivityLineIntervalWrapper aliw: exActivityLineIntervalWrappers){
            ActivityLineIntervalChangeMoveHelper.assignActivityIntervalToShift(scoreDirector,aliw.getActivityLineInterval(),aliw.getShiftRequestPhase());
        }
        log.debug("*P1*********************ENDS********************");
        log.debug("*P2*********************STARTS********************");
        for(ActivityLineIntervalWrapper aliw: activityLineIntervalWrappers){
    ...
    public Collection<?> getPlanningValues() {
        return Collections.singletonList(activityLineIntervalWrappers.get(0).getShiftRequestPhase());
    }
    ...
    public String toString() {
        ...
        return "ActivityLineIntervalPillarMove{" +
                "activityLineIntervals=" + (detailed?(activityLineIntervalWrappers.stream().map(a->a.getActivityLineInterval().getLabel()).collect(Collectors.toList())):activityLineIntervalWrappers.size()) +" to:"+activityLineIntervalWrappers.get(0).getShiftRequestPhase()+
                '}';
    }
}
