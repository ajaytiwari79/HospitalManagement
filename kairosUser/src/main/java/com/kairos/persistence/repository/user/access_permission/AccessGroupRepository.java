...
//            "unwind coll as accessPage with distinct accessPage,accessGroup\n" +
    ...
    @Query("Match (accessPage:AccessPage),(accessGroup:AccessGroup) where id(accessPage)={4} AND id(accessGroup)={3} with accessPage,accessGroup\n" +
            ...
            "DELETE r")
    void deleteCustomPermissionForTabAndChild(long organizationId, long staffId, long unitId, long accessGroupId, long accessPageId);
    ...
    @Query("MATCH (c:Country)-[r:HAS_ACCESS_GROUP]->(ag:AccessGroup{deleted:false}) WHERE id(c)={0} AND r.organizationCategory={1} AND id(ag)={2} \n" +
            "RETURN COUNT(ag)>0")
    ...
    @Query("MATCH (c:Country)-[r:HAS_ACCESS_GROUP]->(ag:AccessGroup{deleted:false}) WHERE id(c)={0} AND r.organizationCategory={1} AND ag.role={2}\n" +
            "RETURN id(ag) as id, ag.name as name, ag.description as description, ag.typeOfTaskGiver as typeOfTaskGiver, ag.deleted as deleted, ag.role as role")
    List<AccessGroupQueryResult> getCountryAccessGroupByOrgCategoryAndRole(Long countryId, String orgCategory, String role);

    @Query("MATCH (org:Organization)-[r:ORGANIZATION_HAS_ACCESS_GROUPS]->(ag:AccessGroup{deleted:false}) WHERE id(org)={0} AND r.organizationCategory={1} AND ag.role={2}\n" +
            "RETURN id(ag) as id, ag.name as name, ag.description as description, ag.typeOfTaskGiver as typeOfTaskGiver, ag.deleted as deleted, ag.role as role")
    List<AccessGroupQueryResult> getOrganizationAccessGroupByOrgCategoryAndRole(Long organizationId, String orgCategory, String role);
