...
public enum AutoFillGapSettingsRule implements Serializable {
    ...
    HIGHEST_RANKED_ACTIVITY_IF_IT_IS_SOLVING_MORE_PROBLEMS_THAN_CAUSING("Fill the gap only with the highest ranked activity if it is solving more problems than causing (planned or not in the shift)."),
    HIGHEST_RANKED_ACTIVITY_AMONGST_THAT_ARE_SOLVING_MORE_PROBLEMS_THAN_CAUSING("The activity to fill the gap will be the one which has the highest rank amongst that are solving more than causing (planned or not in the shift)."),
