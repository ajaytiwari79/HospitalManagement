...
public class VetoAndStopBricksWTATemplate extends WTABaseRuleTemplate {
    ...
    public void validateRules(RuleTemplateSpecificInfo infoWrapper) {
        if (!isDisabled() && CollectionUtils.containsAny(infoWrapper.getShift().getActivitiesIds(),getActivityIds()) && validationStartDate.minusDays(1).isBefore(DateUtils.asLocalDate(infoWrapper.getShift().getStartDate()))) {
            ...
            for (ShiftWithActivityDTO shift : shifts) {
                if(interval.contains(shift.getStartDate())){
                    if (shift.getActivitiesIds().contains(vetoActivityId)) {
                        totalVeto++;
                    } else if (shift.getActivitiesIds().contains(stopBrickActivityId)) {
                        totalStopBricks++;
                    }
                }
            }
