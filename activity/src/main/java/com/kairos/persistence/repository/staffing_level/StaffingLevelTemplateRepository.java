...
public interface StaffingLevelTemplateRepository extends MongoBaseRepository<StaffingLevelTemplate,BigInteger>,CustomStaffingLevelTemplateRepository {
    ...
    List<StaffingLevelTemplateDTO> findByUnitIdDayTypeAndDate(Long unitID, Date proposedStartDate, Date proposedEndDate, List<Long> dayTypeId, List<String> days);

    @Query("{ 'unitId' : ?0 ,disabled:false,deleted:false, 'validity.startDate' : { '$lte' : ?1},'validity.endDate' : { '$gte' : ?2}, 'dayType' :{ '$in' : ?3 } }")
    List<StaffingLevelTemplateDTO> findByUnitIdHolidayDayTypeAndDate(Long unitID, Date proposedStartDate, Date proposedEndDate, List<Long> dayTypeId);
