...
public interface TodoRepository extends MongoBaseRepository<Todo, BigInteger>,CustomTodoRepository {
    ...
    @Query(value = "{unitId:?0,deleted:false,status:{$in:?1},shiftDate:{$gte:?2}}")
    List<TodoDTO> findAllByNotApproved(Long unitId, Collection<TodoStatus> statuses, LocalDate currentDate);
