...
public interface AutoFillGapSettingsMongoRepository extends MongoBaseRepository<AutoFillGapSettings, BigInteger> {
    ...
    @Query("{'deleted' : false,'unitId':?0,'organizationTypeId':?1,'organizationSubTypeId':?2,'phaseId':?3,'gapFillingScenario':?4,'id':{$ne:?5},'gapApplicableFor':?6}")
    ...
    @Query("{'deleted' : false,'countryId':?0,'organizationTypeId':?1,'organizationSubTypeId':?2,'phaseId':?3,'gapFillingScenario':?4,'id':{$ne:?5},'gapApplicableFor':?6}")
