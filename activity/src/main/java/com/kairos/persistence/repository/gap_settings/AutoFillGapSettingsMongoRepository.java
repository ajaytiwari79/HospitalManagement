...
public interface AutoFillGapSettingsMongoRepository extends MongoBaseRepository<AutoFillGapSettings, BigInteger> {
    ...
    @Query(value = "{deleted : false,unitId:?0,phaseId: ?1, $or:[{startDate:{$lte:?2},endDate:{$exists:false} },{startDate: {$lte: ?2},endDate:{$gte:?2}}]}",exists = true)
    Boolean isAutoFillGapSettingsByUnitId(Long unitId, BigInteger phaseId, LocalDate startDate);
