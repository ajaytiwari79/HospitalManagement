...
public class WorkingTimeAgreementMongoRepositoryImpl implements CustomWorkingTimeAgreementMongoRepostory {
    ...
    public boolean wtaExistsByUnitPositionIdAndDates(Long unitPositionId, Date startDate, Date endDate) {
        Criteria endDateCriteria;
        if(isNotNull(endDate)){
            endDateCriteria = Criteria.where("endDate").exists(false).and("startDate").lte(endDate);
        }else {
            endDateCriteria = Criteria.where("endDate").exists(false);
        }
