...
public class ActivityMongoRepositoryImpl implements CustomActivityMongoRepository {
    ...
    public List<ActivityTagDTO> findAllActivityByUnitIdAndDeleted(Long unitId, Long countryId, List<Long> orgSubTypes) {
        Aggregation aggregation = Aggregation.newAggregation(
                ...
                project(NAME, DESCRIPTION, UNIT_ID, RULES_ACTIVITY_TAB,STATE, COUNTRY_PARENT_ID, PARENT_ID, GENERAL_ACTIVITY_TAB, TAGS,TRANSLATIONS).and(BALANCE_SETTINGS_ACTIVITY_TAB_TIME_TYPE_ID).as(BALANCE_SETTINGS_ACTIVITY_TAB_TIME_TYPE_ID).and(TIME_CALCULATION_ACTIVITY_TAB).as(TIME_CALCULATION_ACTIVITY_TAB)
    ...
    public List<ActivityDTO> findAllActivityByCountryAndPriorityFor(long refId, boolean refType, PriorityFor priorityFor) {
        Criteria criteria = Criteria.where(DELETED).is(false).and(refType?COUNTRY_ID:UNIT_ID).is(refId).and(TIME_TYPE1+".priorityFor").is(priorityFor);
        Aggregation aggregation = Aggregation.newAggregation(
                lookup(TIME_TYPE, BALANCE_SETTINGS_ACTIVITY_TAB_TIME_TYPE_ID, UNDERSCORE_ID, TIME_TYPE1),
                unwind(TIME_TYPE1),
                match(criteria),
