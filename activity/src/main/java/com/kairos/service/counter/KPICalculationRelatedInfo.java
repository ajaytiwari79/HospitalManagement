...
public class KPICalculationRelatedInfo {
        ...
        for (YAxisConfig yAxisConfig : yAxisConfigs) {
            switch (yAxisConfig) {
                ...
                case PLANNING_QUALITY_LEVEL:
                    List<Activity> activities = kpiBuilderCalculationService.getActivityMongoRepository().findAllActivitiesByIds(filterBasedCriteria.containsKey(ACTIVITY_IDS) ? KPIUtils.getBigIntegerSet(filterBasedCriteria.get(ACTIVITY_IDS)) : new HashSet<>());
                    activityMap = activities.stream().collect(Collectors.toMap(Activity::getId, v -> v));
                    break;
                case TIME_TYPE:
                    timeTypeMap = kpiBuilderCalculationService.getTimeTypeService().getAllTimeTypeWithItsLowerLevel(UserContext.getUserDetails().getCountryId(), filterBasedCriteria.containsKey(TIME_TYPE) ? KPIUtils.getBigIntegerValue(filterBasedCriteria.get(TIME_TYPE)) : new ArrayList<>());
                    break;
                case PLANNED_TIME:
                    Collection<PlannedTimeType> plannedTimeTypes = kpiBuilderCalculationService.getPlannedTimeTypeService().getAllPlannedTimeByIds(filterBasedCriteria.containsKey(PLANNED_TIME_TYPE) ? KPIUtils.getBigIntegerValue(filterBasedCriteria.get(PLANNED_TIME_TYPE)) : new ArrayList<>());
    ...
    private void loadKpiCalculationRelatedInfo(Map<FilterType, List> filterBasedCriteria, Long organizationId, ApplicableKPI applicableKPI) {
        ...
        DefaultKpiDataDTO defaultKpiDataDTO = kpiBuilderCalculationService.getCounterHelperService().getKPIAllData(applicableKPI, filterDates, staffIds, employmentTypeIds, unitIds, organizationId, getLongValue(filterBasedCriteria.getOrDefault(TAGS, new ArrayList())),filterBasedCriteria);
    ...
    private void updateShiftsDetailsForKPI(Map<FilterType, List> filterBasedCriteria, Long organizationId, List<Long> unitIds, List<TimeSlotDTO> timeSlotDTOS, List<Integer> dayOfWeeksNo) {
        if (!CollectionUtils.containsAny(newHashSet(DELTA_TIMEBANK, ACTUAL_TIMEBANK, STAFF_AGE, STAFFING_LEVEL_CAPACITY), calculationTypes)) {
            ...
            if (filterBasedCriteria.containsKey(FilterType.CALCULATION_TYPE) && CollectionUtils.containsAny(validKPIS, filterBasedCriteria.get(FilterType.CALCULATION_TYPE))) {
                List<Shift> shiftData = kpiBuilderCalculationService.getShiftMongoRepository().findShiftBetweenDurationAndUnitIdAndDeletedFalse(dateTimeIntervals.get(0).getStartDate(), dateTimeIntervals.get(dateTimeIntervals.size() - 1).getEndDate(), isCollectionNotEmpty(unitIds) ? unitIds : newArrayList(organizationId));
                shifts = ObjectMapperUtils.copyCollectionPropertiesByMapper(shiftData, ShiftWithActivityDTO.class);
            } else {
                shifts = kpiBuilderCalculationService.getShiftMongoRepository().findShiftsByShiftAndActvityKpiFilters(staffIds, isCollectionNotEmpty(unitIds) ? unitIds : Arrays.asList(organizationId), new ArrayList<>(), dayOfWeeksNo, dateTimeIntervals.get(0).getStartDate(), dateTimeIntervals.get(dateTimeIntervals.size() - 1).getEndDate(), false);
    ...
    private void getUpdateTodoStatus() {
        if(filterBasedCriteria.containsKey(ACTIVITY_STATUS)&& !XAxisConfig.PERCENTAGE.equals(xAxisConfigs.get(0))) {
            todoDTOS = kpiBuilderCalculationService.getTodoService().getAllTodoByDateTimeIntervalAndTodoStatus(dateTimeIntervals.get(0).getStartDate(), dateTimeIntervals.get(dateTimeIntervals.size() - 1).getEndDate(),filterBasedCriteria.get(ACTIVITY_STATUS));
    ...
    public List<TodoDTO> getTodosByInterval(DateTimeInterval dateTimeInterval, List<TodoDTO> todoDTOS) {
        if(YAxisConfig.PLANNING_QUALITY_LEVEL.equals(yAxisConfigs.get(0))) {
            ...
            return todoDTOS.stream().filter(todoDTO -> dateTimeInterval.containsAndEqualsEndDate(asDate(todoDTO.getShiftDate()))).collect(Collectors.toList());
        }
    }
