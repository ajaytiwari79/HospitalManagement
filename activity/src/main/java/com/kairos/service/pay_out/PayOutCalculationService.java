...
//~ ======================================================================================================================
            ...
            for (ShiftActivityDTO shiftActivity : shiftActivities) {
                ...
                if (ruleTemplateValid) {
                    if (ruleTemplate.getCalculationFor().equals(CalculationFor.SCHEDULED_HOURS)) {
                        ...
                        ctaScheduledOrCompensationMinutes = Math.round(timeBankCalculationService.new CalculatePlannedHoursAndScheduledHours().getAndUpdateCtaBonusMinutes(interval, ruleTemplate, shiftActivity,staffEmploymentDetails));
    ...
    payOutPerShift.getPayOutPerShiftCTADistributions().forEach(payOutPerShiftCTADistribution -> {
        timeBankCalculationService.getCostByByMinutes(employmentWithCtaDetailsHourlyCostMap.get(payOutPerShift.getEmploymentId()),payOutPerShiftCTADistribution.getMinutes(),payOutPerShift.getDate());
    });
});
    }
