...
public class PriorityGroupRulesDataGetterService {
    ...
    public PriorityGroupRuleDataDTO getData(PriorityGroupDTO priorityGroupDTO) {
        ...
        List<DailyTimeBankEntry> dailyTimeBankEntries = timeBankRepository.findAllByEmploymentIdsAndBeforDate(null,null,employmentIds, DateUtils.getISOEndOfWeekDate(maxDate));
    ...
    public void calculateTimeBankAndPlannedHours(Long unitId, Map<Long, List<DailyTimeBankEntry>> employmentDailyTimeBankEntryMap, Map<BigInteger, List<StaffEmploymentQueryResult>> openShiftStaffMap, Map<BigInteger,OpenShift> openShiftMap) {
        for(Map.Entry<BigInteger,List<StaffEmploymentQueryResult>> entry: openShiftStaffMap.entrySet()) {
            ...
            while(staffEmploymentIterator.hasNext()) {
                ...
                timeBank = -1* (int)timeBankCalculationService.calculateDeltaTimeBankForInterval(null,planningPeriodInterval,new DateTimeInterval(DateUtils.getDateFromLocalDate(DateUtils.getDateFromEpoch(staffEmploymentQueryResult.getStartDate())).getTime(),endDate),employmentWithCtaDetailsDTO,new HashSet<>(),dailyTimeBankEntries,false)[0];
                planningPeriodInterval =  planningPeriodService.getPlanningPeriodIntervalByUnitId(unitId);
                deltaTimeBank =  -1 * (int)timeBankCalculationService.calculateDeltaTimeBankForInterval(null,planningPeriodInterval,new DateTimeInterval(startDateDeltaWeek,endDateDeltaWeek),employmentWithCtaDetailsDTO,new HashSet<>(),dailyTimeBankEntries, false)[0];
