...
public class ShiftService extends MongoBaseService {
    ...
    public ShiftWithViolatedInfoDTO createShift(Long unitId, ShiftDTO shiftDTO, String type) {
        ...
        staffAdditionalInfoDTO.getEmployment().setCtaRuleTemplates(ctaResponseDTO.getRuleTemplates());
        return validateAndUpdateFullWeekOrFullDayShift(shiftDTO,activityWrapper,staffAdditionalInfoDTO,unitId);
    }
    ...
    private ShiftWrapper getAllShiftsOfSelectedDate(Long unitId, LocalDate startLocalDate, LocalDate endLocalDate, ViewType viewType) {
        ...
        //Set<BigInteger> escalationFreeShiftIds=shiftValidatorService.getEscalationFreeShifts(assignedShifts.stream().map(ShiftDTO::getId).collect(Collectors.toList()));
        //assignedShifts.forEach(shift->shift.setEscalationResolved(escalationFreeShiftIds.contains(shift.getId())));
