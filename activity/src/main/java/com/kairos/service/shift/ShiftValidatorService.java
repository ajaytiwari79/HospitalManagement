...
import com.kairos.rule_validator.activity.specification.StaffingLevelAndRankSpecification;
...
public class ShiftValidatorService {
    ...
    public RuleTemplateSpecificInfo getRuleTemplateSpecificInfo(PlanningPeriod planningPeriod, Phase phase, ShiftWithActivityDTO shift, WTAQueryResultDTO wtaQueryResultDTO, StaffAdditionalInfoDTO staffAdditionalInfoDTO, Map<BigInteger, ActivityWrapper> activityWrapperMap) {
        ...
        shifts = updateDraftShifts(shifts,null,staffAdditionalInfoDTO.getUserAccessRoleDTO());
    ...
    private List<ShiftWithActivityDTO> updateDraftShifts(List<ShiftWithActivityDTO> shifts,ShiftActionType shiftActionType,UserAccessRoleDTO userAccessRoleDTO) {
        List<ShiftWithActivityDTO> shiftWithActivityDTOS = new ArrayList<>();
        if (ShiftActionType.SAVE_AS_DRAFT.equals(shiftActionType) && userAccessRoleDTO.getManagement()) {
