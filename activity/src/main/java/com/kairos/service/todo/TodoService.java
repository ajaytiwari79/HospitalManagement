...
public class TodoService {
    ...
    public void updateStatusOfShiftActivityIfApprovalRequired(Map<BigInteger, ActivityWrapper> activityWrapperMap,Shift shift, boolean shiftUpdate) {
        if (!shiftUpdate && UserContext.getUserDetails().isManagement()) {
            shift.getActivities().forEach(shiftActivity -> {
                updateStatusIfApprovalRequired(activityWrapperMap, shiftActivity, shift.getPhaseId());
                shiftActivity.getChildActivities().forEach(childActivity -> updateStatusIfApprovalRequired(activityWrapperMap, childActivity, shift.getPhaseId()));
            });
    ...
    private void updateStatusIfApprovalRequired(Map<BigInteger,ActivityWrapper> activityMap, ShiftActivity shiftActivity, BigInteger phaseId) {
        if (activityMap.containsKey(shiftActivity.getActivityId())) {
            ...
            if(isCollectionNotEmpty(activity.getRulesActivityTab().getApprovalAllowedPhaseIds()) && activity.getRulesActivityTab().getApprovalAllowedPhaseIds().contains(phaseId)){
