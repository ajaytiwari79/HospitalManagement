...
public interface ProcessingLegalBasisRepository extends JpaRepository<ProcessingLegalBasisMD, Long> {
    ...
    @Transactional
    @Modifying
    @Query(value = "update ProcessingLegalBasisMD set deleted = true where countryId = ?1 and id IN (?2) and deleted = false")
    Integer updateProcessingLegalBasisStatus(Long countryId, Set<Long> ids);
