...
public interface QuestionnaireTemplateRepository extends CustomGenericRepository<QuestionnaireTemplate> {
    ...
    @Query(value = "Select QT from QuestionnaireTemplate QT where QT.countryId = ?1 and QT.assetType.id = ?2 and QT.templateType = ?3  and QT.subAssetType IS NULL and QT.deleted = false ")
    ...
    @Query(value = "Select QT from QuestionnaireTemplate QT where QT.organizationId = ?1  and QT.assetType.id = ?2 and QT.subAssetType.id = ?3 and QT.templateType = ?4 and QT.deleted = false and QT.templateStatus = ?5")
    ...
    @Query(value = "Select QT from QuestionnaireTemplate QT where QT.countryId = ?1 and QT.assetType.id = ?2 and QT.subAssetType.id = ?3  and QT.templateType = ?4  and QT.deleted = false ")
   ...
   /* @Modifying
    ...
    @Query(value = "Select QT from QuestionnaireTemplate QT where QT.organizationId = ?1 and QT.deleted = false order by createdAt desc")
