...
public class CustomOAuthAuthenticationProcessingFilter extends OAuth2AuthenticationProcessingFilter {
    ...
    private OAuth2Authentication loadAuthentication(String accessToken, HttpServletRequest request) {
        ...
        if (token == null) {
            ...
            boolean tokenRemoved=redisService.removeUserTokenFromRedisByClientIpAddress(authentication.getUserAuthentication().getName(), request.getRemoteAddr());
            if (!tokenRemoved)
                exceptionService.internalError("unable to remove token from redis");
