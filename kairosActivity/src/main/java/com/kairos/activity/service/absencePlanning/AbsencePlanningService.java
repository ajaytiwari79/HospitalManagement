package com.kairos.activity.service.absencePlanning;

import com.kairos.activity.client.AbsenceTypeRestClient;
...
import com.kairos.activity.persistence.model.task.TaskReport;
...
import com.kairos.activity.persistence.model.task_type.AddressCode;
...
import com.kairos.activity.persistence.repository.common.CustomAggregationQuery;
...
import static com.kairos.activity.constants.AppConstants.STAFF_MISSING_MESSAGE;
import static com.kairos.activity.constants.AppConstants.STAFF_MISSING_STATUS;
...
public class AbsencePlanningService {
    ...
    private  @Autowired AbsenceTypeRestClient absenceTypeRestClient;
    private @Autowired
           ...
           // Organization organization = organizationGraphRepository.findOne(unitId, 2);
            for(TaskDTO taskData : taskList) {
                ...
                if (!taskType1.getTaskTypeVisibility().equals(taskType2.getTaskTypeVisibility()) || !taskType1.getTaskTypeSchedule().equals(taskType2.getTaskTypeSchedule())) {
                    ...
                    if(taskData.getStartAddress() != null)    task.setStartAddress(AddressCode.valueOf(taskData.getStartAddress()));
                    if(taskData.getEndAddress() != null)    task.setEndAddress(AddressCode.valueOf(taskData.getEndAddress()));
    ...
    public void createReport(Task task, TaskDTO taskDTO){
        ...
        if(task.getStartDate().compareTo(taskDTO.getStartDate()) != 0)  taskReport.setCurrentFrom(currentStartDateTime.toString("yyyy MMM dd, hh:mm:ss"));
        if(task.getEndDate().compareTo(taskDTO.getEndDate()) != 0) taskReport.setCurrentTo(currentEndDateTime.toString("yyyy MMM dd, hh:mm:ss"));
        if(task.getDuration().compareTo(taskDTO.getDuration()) != 0) taskReport.setCurrentDuration(taskDTO.getDuration().toString());
        if(!task.getTaskTypeId().toString().equals(taskDTO.getTaskTypeId().toString())) taskReport.setCurrentActivity(taskTypeMongoRepository.findOne(new BigInteger(taskDTO.getTaskTypeId().toString())).getTitle());
    ...
    public void createAndSyncSubTask(Task task, OrganizationDTO organization){
        try {
            ...
            if(task.getStartAddress().toString().equals(task.getEndAddress().toString()) == false) {
                ...
                taskDTO.copyProperties(subTask);
                syncPartialAbsencesTask(taskDTO, organization);
