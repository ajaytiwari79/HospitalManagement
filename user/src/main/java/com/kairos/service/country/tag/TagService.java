...
public class TagService {
    ...
    public Tag addCountryTag(Long countryId, TagDTO tagDTO) {
        ...
        Tag tag = new Tag(tagDTO.getName(), tagDTO.getMasterDataType(), true, tagDTO.getOrgTypeId(), tagDTO.getOrgSubTypeIds(), tagDTO.getColor());
    ...
    private void mappingTagAtOrganization(TagDTO tagDTO) {
        ...
        if(isCollectionNotEmpty(organizations)) {
            for(Organization org : organizations) {
                Tag tag = new Tag(tagDTO.getName(), tagDTO.getMasterDataType(), false, new PenaltyScore(PenaltyScoreLevel.SOFT,0), tagDTO.getColor());
    ...
    public List<Tag> getCountryTagByOrgSubTypes(Long countryId, List<Long> orgSubTypeId){
        ...
        for(Tag tag : country.getTags().stream().filter(tag -> MasterDataTypeEnum.STAFF.equals(tag.getMasterDataType()) && CollectionUtils.containsAny(tag.getOrgSubTypeIds(),orgSubTypeId)).collect(Collectors.toList())){
            tags.add(new Tag(tag.getName(),tag.getMasterDataType(),false,new PenaltyScore(PenaltyScoreLevel.SOFT,0),tag.getColor()));
        }
        return  tags;
    }
    ...
    public Tag addOrganizationTag(Long organizationId, TagDTO tagDTO) {
        ...
        Tag tag = new Tag(tagDTO.getName(), tagDTO.getMasterDataType(), false, ObjectMapperUtils.copyPropertiesByMapper(tagDTO.getPenaltyScore(), PenaltyScore.class), tagDTO.getColor());
