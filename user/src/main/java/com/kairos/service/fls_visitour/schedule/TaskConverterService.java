...
public class TaskConverterService {
  ...
  /*  @Inject
    ...
    @Inject
    VisitourGraphRepository visitourGraphRepository;
    ...
    public void generateTask(Task task, int functionCode, String relationExtID, Map<String, String> flsCredentials) {
        ...
        Client client = clientGraphRepository.findOne(task.getCitizenId());
        ...
        if (task.getSlaStartDuration() > 0)
            timeFrameInfo.put("timeFrom", new Date(task.getTimeFrom().getTime() - (task.getSlaStartDuration() * 60000)));
        else
