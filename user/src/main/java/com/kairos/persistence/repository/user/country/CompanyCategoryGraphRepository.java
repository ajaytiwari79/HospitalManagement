...
public interface CompanyCategoryGraphRepository extends Neo4jBaseRepository<CompanyCategory, Long> {
    ...
    @Query("MATCH (basic_details:Country)-[:" + BELONGS_TO + "]-(companyCategory:CompanyCategory{deleted:false}) where id(basic_details)={0} return id(companyCategory) as id, companyCategory.name as name," +
            " companyCategory.description as description ")
    List<CompanyCategoryResponseDTO> findCompanyCategoriesByCountry(Long countryId);

    @Query("MATCH (basic_details:Country)-[:" + BELONGS_TO + "]-(companyCategory:CompanyCategory{deleted:false}) where id(basic_details)={0} AND id(companyCategory)= {1} return companyCategory")
    CompanyCategory findByCountryAndCompanycategory(Long countryId, Long companyCategoryId);

    @Query("MATCH (basic_details:Country)-[:" + BELONGS_TO + "]-(companyCategory:CompanyCategory{deleted:false}) where id(basic_details)={0} AND id(companyCategory) <> {1} AND companyCategory.name=~{2} " +
