...
public interface EmploymentGraphRepository extends Neo4jBaseRepository<Employment, Long> {
    ...
    @Query("MATCH (unitPosition:Employment{deleted:false}) where id(unitPosition)={0} \n" +
    ...
    EmploymentQueryResult getUnitPositionById(Long unitPositionId);
    ...
    @Query("MATCH(staff:Staff{deleted:false})-[:"+BELONGS_TO+"]-(user:User) WHERE id(staff) IN {2}\n" +
            ...
            "MATCH(staff)-[:" + BELONGS_TO_STAFF + "]->(unitPosition:Employment)-[:" + IN_UNIT + "]->(unit:Organization) where id(unit)={0}\n " +
    ...
    @Query("MATCH (unitPosition:Employment{deleted:false}) where id(unitPosition) IN {0} \n" +
    ...
    List<EmploymentQueryResult> getUnitPositionByIds(List<Long> unitPositionIds);



    @Query("MATCH(s:Staff)-[:" + BELONGS_TO_STAFF + "]-(unitPosition:Employment{deleted:false,published:true})-[:" + IN_UNIT + "]-(o:Organization) where id(o)={0} AND id(s)={1} \n" +
            "MATCH(unitPosition)-[:HAS_EXPERTISE_IN]-(e:Expertise) where id(e)={2}\n" +
            "return unitPosition ORDER BY unitPosition.startDate")
    List<Employment> getStaffUnitPositionsByExpertise(Long unitId, Long staffId, Long expertiseId);
    ...
    @Query("MATCH (user:User)-[:"+BELONGS_TO+"]-(staff:Staff) where id(user)={0}\n" +
            ...
            "OPTIONAL MATCH(subOrg)<-[:"+IN_UNIT+"]-(unitPosition:Employment{deleted:false})<-[:"+BELONGS_TO_STAFF+"]-(staff) WITH unitPosition,org,subOrg,staff,position \n" +
            "MATCH(unitPosition)-[:"+HAS_EXPERTISE_IN+"]->(expertise:Expertise) \n" +
            ...
            "MATCH(staff)<-[:"+BELONGS_TO+"]-(position:Position)<-[:"+ HAS_POSITIONS +"]-(org:Organization) \n" +
            "MATCH(org)<-[:"+IN_UNIT+"]-(unitPosition:Employment{deleted:false})<-[:"+BELONGS_TO_STAFF+"]-(staff)  \n" +
    ...
    List<EmploymentQueryResult> getAllUnitPositionsByUser(long userId);
...
// Date is not supported as a return type in Bolt protocol version 1. Please make sure driver supports at least protocol version 2. Driver upgrade is most likely required
    @Query("MATCH(staff:Staff)-[:" + BELONGS_TO_STAFF + "]->(up:Employment{deleted:false}) where id(staff)={0} return up.endDate as endDate")
    List<String> getAllUnitPositionsByStaffId(Long staffId);

    @Query("MATCH(unitPosition:Employment)-[:" + IN_UNIT + "]->(subOrg:Organization) where id(unitPosition)={0} " +
            "MATCH(unitPosition)<-[:" + BELONGS_TO_STAFF + "]-(staff:Staff) " +
    ...
    EmploymentQueryResult getUnitIdAndParentUnitIdByUnitPositionId(Long unitPositionId);
    ...
    @Query("MATCH(staff:Staff)-[:" + BELONGS_TO_STAFF + "]->(up:Employment{deleted:false}) where id(staff)={0} AND ( up.endDate IS NULL OR DATE(up.endDate) > DATE({1}))  return up")
    List<Employment> getUnitPositionsFromEmploymentEndDate(Long staffId, String endDate);

    @Query("MATCH(staff:Staff)-[:" + BELONGS_TO_STAFF + "]->(up:Employment) where id(up)={0} return id(staff) as staffId")
    Long getStaffIdFromUnitPosition(Long unitPositionId);

    @Query("MATCH(staff:Staff)-[:" + BELONGS_TO_STAFF + "]->(up:Employment{deleted:false}) where id(staff)={0} return max(up.startDate) as maxStartDate")
    String getMaxUnitPositionStartDate(Long staffId);

    @Query("MATCH(org:Organization)<-[:"+IN_UNIT+"]-(unitPosition:Employment{deleted:false})<-[:"+BELONGS_TO_STAFF+"]-(staff) WHERE id(staff)={0} AND id(org)={1}\n" +
            "MATCH(unitPosition)-[:"+HAS_EXPERTISE_IN+"]->(expertise:Expertise)  \n" +
            "OPTIONAL MATCH (unitPosition)-[:"+HAS_REASON_CODE+"]->(reasonCode:ReasonCode) \n" +
            "OPTIONAL MATCH (expertise)-[:"+SUPPORTED_BY_UNION+"]->(unionData:Organization{isEnable:true,union:true}) \n" +
    ...
    List<EmploymentQueryResult> getAllUnitPositionsForCurrentOrganization(long staffId, Long unitId);

    @Query("MATCH(unitPosition:Employment{deleted:false,published:true})-[:" + HAS_EXPERTISE_IN + "]->(expertise:Expertise) where id(expertise)={2}\n" +
            "MATCH(unitPosition)-[:"+ HAS_EMPLOYMENT_LINES +"]-(employmentLine:EmploymentLine) WHERE  date(employmentLine.startDate) <= date() AND (NOT exists(employmentLine.endDate) OR date(employmentLine.endDate) >= date())\n"+
    ...
    @Query("MATCH (unit:Organization) WHERE id(unit)={0} \n" +
            "MATCH (unit)<-[:" + IN_UNIT + "]-(unitPosition:Employment{deleted:false,published:true})-[:" + HAS_EXPERTISE_IN + "]-(expertise:Expertise) \n" +
    ...
    @Query("MATCH (user:User)-[:BELONGS_TO]-(staff:Staff)<-[:" + BELONGS_TO + "]-(position:Position)<-[:"+HAS_POSITIONS+"]-(org:Organization) where id(user)={0}\n" +
            "MATCH(org)-[:"+HAS_SUB_ORGANIZATION+"*]->(subOrg:Organization)\n" +
            "MATCH(subOrg)<-[:"+IN_UNIT+"]-(unitPosition:Employment{deleted:false,published:true})<-[:"+BELONGS_TO_STAFF+"]-(staff)\n" +
            ...
            "MATCH (user:User)-[:"+BELONGS_TO+"]-(staff:Staff) where id(user)={0}\n" +
            "MATCH(staff)<-[:"+BELONGS_TO+"]-(position:Position)<-[:"+HAS_POSITIONS+"]-(org:Organization) \n" +
            "MATCH(org)<-[:"+IN_UNIT+"]-(unitPosition:Employment{deleted:false,published:true})<-[:"+BELONGS_TO_STAFF+"]-(staff)  \n" +
            "return id(unitPosition) as id, unitPosition.history as history,\n" +
            "id(org) as parentUnitId,id(org) as unitId,{id:id(org),name:org.name} as unitInfo ORDER BY unitPosition.creationDate")
    List<EmploymentQueryResult> getAllUnitPositionsBasicDetailsAndWTAByUser(long userId);
    ...
    @Query( "MATCH(sector:Sector)-[:"+BELONGS_TO_SECTOR+"]-(expertise:Expertise{deleted:false}) \n" +
            ...
            "MATCH(activeEmploymentLine:EmploymentLine)<-[:"+ HAS_EMPLOYMENT_LINES +"]-(unitPosition:Employment{deleted:false,published:true})-[:"+HAS_EXPERTISE_IN+"]->(expertise) WHERE activeEmploymentLine.endDate IS NULL OR activeEmploymentLine.endDate >= date()\n" +
    ...
    List<EmploymentSeniorityLevelQueryResult> findUnitPositionSeniorityLeveltoUpdate();

    @Query("MATCH(staff:Staff),(unit:Organization) where id(staff)={0} and id(unit)={1} \n" +
            "MATCH(unit)<-[:" + IN_UNIT + "]-(unitPosition:Employment{deleted:false,published:true})<-[:" + BELONGS_TO_STAFF + "]-(staff)" +
            "MATCH(unitPosition)-[:"+ HAS_EMPLOYMENT_LINES +"]-(employmentLine:EmploymentLine) WHERE  date(employmentLine.startDate) <= date() AND (NOT exists(employmentLine.endDate) OR date(employmentLine.endDate) >= date()) \n" +
            "MATCH(unitPosition)-[:" + HAS_EXPERTISE_IN + "]->(expertise:Expertise) \n" +
            "MATCH(employmentLine)-[employmentRel:" + HAS_EMPLOYMENT_TYPE + "]->(employmentType:EmploymentType) \n" +
    ...
    @Query("MATCH(unitPosition:Employment) where id(unitPosition)={0} return unitPosition.published as published ," +
            "unitPosition.startDate as startDate, unitPosition.endDate as endDate, id(unitPosition) as id")
    EmploymentQueryResult findByUnitPositionId(Long unitPositionId);

    @Query(" MATCH (unitPosition:Employment) where id(unitPosition) IN {0} " +
            "MATCH(unitPosition)-[:" + HAS_EMPLOYMENT_LINES + "]-(employmentLine:EmploymentLine) " +
...
@Query("OPTIONAL MATCH (organization:Organization)  WHERE id(organization)={0}\n" +
        ...
        "OPTIONAL MATCH(staff)-[:"+BELONGS_TO_STAFF+"]->(unitPosition:Employment{published:true})-[:"+IN_UNIT+"]->(organization) " +
    ...
    @Query("MATCH(unitPosition:Employment{published:true})-[unitPositionOrgRel:"+ IN_UNIT +"]-(organization:Organization) \n" +
    ...
    @Query("OPTIONAL MATCH(organization:Organization) WHERE id(organization)={0}\n" +
            "OPTIONAL MATCH(staff:Staff) WHERE id(staff)={1}\n" +
            "OPTIONAL MATCH(unitPosition:Employment) WHERE id(unitPosition)={2}\n" +
            "OPTIONAL MATCH(unitPosition:Employment{published:true})-[unitPositionOrgRel: IN_UNIT ]-(organization) \n" +
    ...
    @Query("MATCH(staff:Staff)-[:"+BELONGS_TO+"]->(user:User)  where id(staff)={0}\n" +
            ...
            "OPTIONAL MATCH(staffList)-[:"+BELONGS_TO_STAFF+"]->(up:Employment{mainUnitPosition:TRUE}) WHERE id(up)<> {3} AND  " +
