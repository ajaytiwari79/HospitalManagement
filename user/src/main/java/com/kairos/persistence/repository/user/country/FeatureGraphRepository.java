...
public interface FeatureGraphRepository extends Neo4jBaseRepository<Feature,Long> {
    ...
    @Query("Match (basic_details:Country)-[r:"+COUNTRY_HAS_FEATURE+"]->(feature:Feature)\n" +
            "WHERE feature.name={0} AND id(basic_details) = {1} AND feature.deleted={2} \n" +
    ...
    @Query("Match (basic_details:Country)-[r:"+COUNTRY_HAS_FEATURE+"]->(feature:Feature)\n" +
            "WHERE id(feature)={0} AND id(basic_details) = {1} AND feature.deleted={2} \n" +
            "RETURN feature")
    Feature getFeatureById(Long tagId, Long countryId, boolean isDeleted);

    @Query("Match (basic_details:Country)-[r:"+COUNTRY_HAS_FEATURE+"]->(feature:Feature)\n" +
            "WHERE id(feature)={0} AND id(basic_details) = {1} \n" +
    ...
    @Query("Match (basic_details:Country)-[r:"+COUNTRY_HAS_FEATURE+"]->(feature:Feature)\n" +
            "WHERE id(basic_details)={0} AND feature.deleted= {1} AND lower(feature.name) contains lower({2})\n" +
            "return id(feature) as id, feature.name as name, feature.description as description")
    List<FeatureQueryResult> getListOfFeatures(Long countryId , boolean deleted, String searchTextRegex);

    @Query("Match (basic_details:Country)-[r:"+COUNTRY_HAS_FEATURE+"]->(feature:Feature)\n" +
            "WHERE id(basic_details)={0} AND feature.deleted= {1} AND id(feature) IN {2}\n" +
            "return feature")
    List<Feature> getListOfFeaturesByCountryAndIds(Long countryId , boolean deleted, List<Long> featureIds);

    @Query("Match (basic_details:Country)-[r:"+COUNTRY_HAS_FEATURE+"]->(feature:Feature)\n"+
            "WHERE id(basic_details)={0} AND feature.name = {1} AND feature.deleted= {2} return feature")
    ...
    @Query("Match (o:Organization)-[:"+COUNTRY+"]->(basic_details:Country)-[r:"+COUNTRY_HAS_FEATURE+"]->(feature:Feature)\n" +
