...
public interface UnitPermissionGraphRepository extends Neo4jBaseRepository<UnitPermission, Long> {
    ...
    @Query("MATCH (organization:Unit),(staff:Staff) WHERE id(organization)={1} AND id(staff)={0} WITH organization,staff\n" +
    ...
    @Query("MATCH (organization:Unit),(staff:Staff) WHERE id(organization)={0} AND id(staff)={2} WITH organization,staff\n" +
            "MATCH (organization)-[:" + HAS_POSITIONS + "]->(position:Position)-[:"+BELONGS_TO+"]->(staff) WITH position\n" +
            "MATCH (position)-[:" + HAS_UNIT_PERMISSIONS + "]->(unitPermission:UnitPermission) WITH unitPermission\n" +
            "MATCH (unitPermission)-[r:" + APPLICABLE_IN_UNIT + "]->(unit:Unit) WHERE id(unit)={1} WITH unitPermission\n" +
    ...
    @Query("MATCH (organization:Unit),(user:User) WHERE id(organization)={0} AND id(user) ={1}\n" +
            "MATCH(organization)<-[:"+HAS_SUB_ORGANIZATION+"]-(parent:Unit)\n" +
