...
public interface StaffGraphRepository extends Neo4jBaseRepository<Staff, Long>, CustomStaffGraphRepository {
    ...
    @Query("MATCH (unitPermission:UnitPermission)-[:"+APPLICABLE_IN_UNIT+"]->(organization:Organization)<-[:"+IN_UNIT+"]-(unitPosition:Employment) WHERE  id(unitPosition)={0} WITH unitPermission,organization,unitPosition \n" +
    ...
    StaffAdditionalInfoQueryResult getStaffInfoByUnitIdAndEmploymentId(long unitPositionId);
    ...
    @Query("MATCH (staff:Staff) WHERE id(staff) IN {1}  " +
            ...
            "MATCH(staff)-[:" + BELONGS_TO_STAFF + "]->(unitPosition:Employment)-[:" + IN_UNIT + "]->(organization) \n " +
            "MATCH(unitPosition)-[:"+ HAS_EMPLOYMENT_LINES +"]-(employmentLine:EmploymentLine) WHERE  NOT EXISTS(employmentLine.endDate) OR date(employmentLine.endDate) >= date() WITH staff,organization " +
    ...
    @Query("MATCH(staff:Staff)-[:"+BELONGS_TO_STAFF+"]-(unitPos:Employment{deleted:false})-[:"+IN_UNIT+"]-(organization:Organization) WHERE id(organization)={0}\n" +
    ...
    List<StaffPersonalDetailDTO> getAllStaffHavingEmploymentByUnitId(long unitId, String imageUrl);
    ...
    List<StaffPersonalDetailDTO> getAllStaffHavingEmploymentByUnitIdMap(long unitId, String imageUrl);

    @Query("MATCH (organization:Organization)-[:"+HAS_POSITIONS+"]-(position:Position)-[:BELONGS_TO]-(staff:Staff) WHERE id(organization)={0} \n" +
            "MATCH (staff)-[:"+BELONGS_TO+"]->(user:User) WITH user, staff\n" +
            "OPTIONAL MATCH(staff)-[:BELONGS_TO_STAFF]-(unitPos:Employment{deleted:false})-[:IN_UNIT]-(organization)" +
...
// TODO CRITICAL ISSUE we are fetching all staff across all organisation i think it should be refactored
    ...
    @Query("MATCH(staff:Staff)-[:BELONGS_TO_STAFF]-(unitPosition:Employment{deleted:false}) WHERE staff.externalId={0} AND unitPosition.timeCareExternalId={1} " +
    ...
    @Query("MATCH(staff:Staff)-[: "+BELONGS_TO_STAFF+" ]-(unitPosition:Employment{deleted:false})-[:"+IN_UNIT+"]-(organization:Organization) WHERE id(organization) IN {0} " +
    ...
    @Query("MATCH (org:Organization) WITH org\n" +
            ...
            "MATCH (staff)-[:"+BELONGS_TO_STAFF+"]-(unitPosition:Employment{deleted:false})-[:"+IN_UNIT+"]-(o:Organization)\n" +
    ...
    @Query("MATCH(staff:Staff)-[:"+BELONGS_TO_STAFF+"]->(unitPosition:Employment)-[:"+HAS_EXPERTISE_IN+"]->(expertise:Expertise) WHERE id(expertise)={1} AND id(staff) IN {2}\n" +
    ...
    List<StaffEmploymentDetails> getStaffIdAndUnitPositionId(Long unitId, Long expertiseId, List<Long> staffIds, Long currentMillis);
    ...
    @Query("MATCH (organization:Organization{deleted:false,isEnable:true})-[:"+ HAS_POSITIONS +"]->(position:Position)-[:"+BELONGS_TO+"]-(staff:Staff) " +
            ...
            "WITH staff OPTIONAL MATCH (staff)-[:" + BELONGS_TO_STAFF +"]-(unitPos:Employment{deleted:false})-[: "+ IN_UNIT + "]-(organization:Organization) WHERE id(organization)={1} " +
            "RETURN distinct id(staff) AS id ,staff.lastName AS lastName  , staff.firstName AS firstName ,staff.profilePic AS profilePic, staff.email AS email , staff.userName AS userName ,staff.access_token AS access_token ,staff.user_id AS user_id")
