...
// TODO CRITICAL ISSUE we are fetching all staff across all organisation i think it should be refactored
    ...
    @Query("Optional MATCH (organization:Organization)-[:" + HAS_EMPLOYMENTS + "]-(e:Employment)-[:" + BELONGS_TO + "]-(staff:Staff) WHERE id(organization)={1} with staff  " +
            "MATCH (staff)-[:" + HAS_CONTACT_DETAIL + "]-(contactDetail:ContactDetail)  WHERE staff.email=~{0} OR contactDetail.privateEmail=~{0}  RETURN staff")
