...
public interface AccessPermissionGraphRepository extends Neo4jBaseRepository<AccessPermission,Long> {
    ...
    @Query("Match (organization:Organization),(staff:Staff),(accessGroup:AccessGroup),(accessPage:AccessPage) where id(organization)={0} AND id(staff)={1} AND id(accessGroup)={2} AND id(accessPage)={3} with organization,staff,accessGroup,accessPage\n" +
            "Match (organization)-[:HAS_EMPLOYMENTS]->(employment:Employment)-[:BELONGS_TO]->(staff)-[:HAS_UNIT_PERMISSIONS]->(unitEmployment:UnitEmployment) with unitEmployment,accessGroup,accessPage\n" +
            "Match (employment)-[:HAS_UNIT_PERMISSIONS]->(unitEmployment:UnitEmployment)-[:HAS_ACCESS_PERMISSION]->(accessPermission:AccessPermission)-[:HAS_ACCESS_GROUP]->(accessGroup) with accessPermission,accessPage\n" +
