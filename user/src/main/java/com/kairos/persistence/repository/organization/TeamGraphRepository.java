...
public interface TeamGraphRepository extends Neo4jBaseRepository<Team,Long>{
    ...
    @Query("MATCH (team:Team) WHERE id(team)={0} with team\n" +
            "OPTIONAL MATCH (team)-[staffRel:"+TEAM_HAS_MEMBER+"]->(teamMembers:Staff) where staffRel.teamMember = true\n" +
            "WITH team,COLLECT(DISTINCT{staffId:id(teamMembers),teamType:staffRel.teamType,startDate:staffRel.startDate,endDate:staffRel.endDate,sequence:staffRel.sequence,teamMember:staffRel.teamMember}) as staffDetails\n" +
    ...
    @Query("MATCH (team:Team{isEnabled:true})-[staffTeamRel:"+TEAM_HAS_MEMBER+"]->(staff:Staff) WHERE id(team)={0} AND EXISTS(staffTeamRel.leaderType) AND staffTeamRel.teamMember=false AND NOT id(staff) IN {1} DETACH DELETE staffTeamRel")
    ...
    @Query("MATCH (organization:Unit)-[:"+HAS_TEAMS+"]->(team:Team{isEnabled:true,deleted:false})-[staffTeamRel:"+TEAM_HAS_MEMBER+"]->(staff:Staff) WHERE id(staff)={0} AND id(organization)={1} RETURN \n" +
            "id(team) as id,team.name as name,staffTeamRel.teamType as teamType,staffTeamRel.leaderType as leaderType,staffTeamRel.sequence as sequence,staffTeamRel.teamMember as teamMember")
