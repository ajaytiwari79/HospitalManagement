...
public interface UnitPositionGraphRepository extends Neo4jBaseRepository<UnitPosition, Long> {
   ...
   /* @Query("Match (org:Organization) where id(org)={0}\n" +
    ...
    @Query("MATCH (user:User)-[:BELONGS_TO]-(staff:Staff) where id(user)={0}\n" +
            ...
            "unitPosition.hourlyWages as hourlyWages,CASE when pgaRel.payGroupAreaAmount IS NULL THEN 0 ELSE ((pgaRel.payGroupAreaAmount/52)*unitPosition.fullTimeWeeklyMinutes) END as hourlyCost, id(unitPosition) as id,unitPosition.avgDailyWorkingHours as avgDailyWorkingHours,\n" +
            ...
            "{employmentTypeCategory:employmentRel.employmentTypeCategory,name:employmentType.name,id:id(employmentType)} as employmentType, \n" +
            "unitPosition.hourlyWages as hourlyWages,CASE when pgaRel.payGroupAreaAmount IS NULL THEN 0 ELSE ((pgaRel.payGroupAreaAmount/52)*unitPosition.fullTimeWeeklyMinutes) END as hourlyCost, id(unitPosition) as id,\n" +
            "unitPosition.avgDailyWorkingHours as avgDailyWorkingHours,\n" +
