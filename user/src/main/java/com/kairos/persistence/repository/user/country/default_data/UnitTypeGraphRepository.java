...
public interface UnitTypeGraphRepository extends Neo4jBaseRepository<UnitType,Long> {
    @Query("match(country:Country)-[rel:"+IN_COUNTRY+"]-(unitType:UnitType{deleted:false}) where id(country)={0} AND unitType.name ~={1} AND id(unitType)<>{0} " +
            " with count(unitType) as totalCount " +
            " RETURN CASE WHEN toatalCount>0 THEN TRUE ELSE FALSE END as result")
    Boolean checkUnitTypeExistInCountry(Long countryId,String name,Long currentUnitTypeId );

    @Query("match(country:Country)-[rel:"+IN_COUNTRY+"]-(unitType:UnitType{deleted:false}) where id(country)={0} " +
