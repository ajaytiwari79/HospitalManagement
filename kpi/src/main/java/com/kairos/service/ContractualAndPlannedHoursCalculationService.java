...
public class ContractualAndPlannedHoursCalculationService implements CounterService {
    ...
    private List<CommonKpiDataUnit> getContractualAndPlannedHoursKpiDate(Long organizationId, Map<FilterType, List> filterBasedCriteria ,ApplicableKPI applicableKPI) {
        ...
        List<ShiftDTO> shifts = counterHelperRepository.findShiftsByKpiFilters((List<Long>) kpiData[2], ObjectUtils.isCollectionNotEmpty(unitIds) ? unitIds : Arrays.asList(organizationId), new ArrayList<>(), new HashSet<>(), dateTimeIntervals.get(0).getStartDate(), dateTimeIntervals.get(dateTimeIntervals.size() - 1).getEndDate());
        ...
        for (EmploymentWithCtaDetailsDTO positionWithCtaDetailsDTO : staffKpiFilterDTO.getEmployment()) {
            if (interval != null) {
                ...
                while (startDate.isBefore(interval.getEnd())) {
                    contractualMinutes += timeBankService.getContractualMinutesByDate(planningPeriodInterval, DateUtils.asLocalDate(startDate), positionWithCtaDetailsDTO.getEmploymentLines());
    ...
    public Map<Object, Double> calculateContractualHoursPerInterval(List<StaffKpiFilterDTO> staffKpiFilterDTOS ,List<DateTimeInterval> dateTimeIntervals, DurationType frequencyType) {
        ...
        for (StaffKpiFilterDTO staffKpiFilterDTO : staffKpiFilterDTOS) {
            ...
            dateTimeIntervalAndContractualHourMap.put(DurationType.DAYS.equals(frequencyType) ? DateUtils.getStartDateTimeintervalString(dateTimeInterval) : DateUtils.getDateTimeintervalString(dateTimeInterval), DateUtils.getHoursByMinutes(contractualMinutes.doubleValue()));
    ...
    public Map<Object, Double> calculateContractualHoursTotalData(Interval interval, List<StaffKpiFilterDTO> staffKpiFilterDTOS ,List<DateTimeInterval> dateTimeIntervals) {
        Map<Object, Double> dateTimeIntervalAndContractualHourMap = new HashMap<>();
            ...
            dateTimeIntervalAndContractualHourMap.put(DateUtils.getDateTimeintervalString(new DateTimeInterval(dateTimeIntervals.get(0).getStartDate(), dateTimeIntervals.get(dateTimeIntervals.size() - 1).getEndDate())), DateUtils.getHoursByMinutes(contractualMinutes.doubleValue()));
    ...
    private Map<Object, Double> calculateDataByKpiRepresentation(List<DateTimeInterval> dateTimeIntervals, ApplicableKPI applicableKPI,List<StaffKpiFilterDTO> staffKpiFilterDTOS){
        ...
        return KPIUtils.verifyKPIResponseData(staffContratualHours) ? staffContratualHours : new HashMap<>();
    }
