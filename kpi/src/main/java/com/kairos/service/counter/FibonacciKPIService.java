...
public class FibonacciKPIService implements CounterService{
    ...
    public FibonacciKPIDTO createFibonacciKPI(Long referenceId, FibonacciKPIDTO fibonacciKPIDTO, ConfLevel confLevel) {
        ...
        ApplicableKPI applicableKPI=new ApplicableKPI(fibonacciKPI.getId(), fibonacciKPI.getId(), ConfLevel.COUNTRY.equals(confLevel)?referenceId:null, ConfLevel.UNIT.equals(confLevel)?referenceId:null, null, confLevel, new ApplicableFilter(new ArrayList<>(), false), fibonacciKPI.getTitle(), false,ObjectMapperUtils.copyCollectionPropertiesByMapper(fibonacciKPIDTO.getFibonacciKPIConfigs(),FibonacciKPIConfig.class),KPIRepresentation.INDIVIDUAL_STAFF);
        applicableKPIRepository.save(applicableKPI);
        KPICategory kpiCategory=counterRepository.getKPICategoryByName(AppConstants.UNCATEGORIZED,confLevel,referenceId);
        CategoryKPIConf categoryKPIConf=new CategoryKPIConf(applicableKPI.getActiveKpiId(), kpiCategory.getId(), ConfLevel.COUNTRY.equals(confLevel)?referenceId:null, ConfLevel.UNIT.equals(confLevel)?referenceId:null, confLevel);
    ...
    public KPIDTO getOneFibonacciKPI(BigInteger fibonacciKPIId,Long referenceId,ConfLevel confLevel){
        ...
        if(confLevel.equals(ConfLevel.UNIT)){
    ...
    private KPIResponseDTO calculateFibonacciKPI(Map<FilterType, List> filterBasedCriteria, Long organizationId, ApplicableKPI applicableKPI) {
        List<LocalDate> filterDates = (filterBasedCriteria.get(FilterType.TIME_INTERVAL) != null) && ObjectUtils.isCollectionNotEmpty(filterBasedCriteria.get(FilterType.TIME_INTERVAL)) ? KPIUtils.getLocalDate(filterBasedCriteria.get(FilterType.TIME_INTERVAL)) : Arrays.asList(DateUtils.getStartDateOfWeek(), DateUtils.getEndDateOfWeek());
        StaffEmploymentTypeDTO staffEmploymentTypeDTO = new StaffEmploymentTypeDTO(new ArrayList<>(), ObjectUtils.newArrayList(organizationId), new ArrayList<>(), organizationId, filterDates.get(0).toString(), filterDates.get(1).toString(),new ArrayList<>(),filterBasedCriteria,true);
    ...
    private Map<Long, FibonacciKPICalculation> getCalculationByCounters(Map<FilterType, List> filterBasedCriteria, Long organizationId, ApplicableKPI applicableKPI, List<StaffKpiFilterDTO> staffKpiFilterDTOS, Map<BigInteger, FibonacciKPIConfig> fibonacciKPIConfigMap, List<KPI> counters, Map<BigInteger, ApplicableKPI> applicableKPIMap) {
        ...
        for (KPI counter : counters) {
            if(applicableKPIMap.containsKey(counter.getId())) {
                if(CounterType.ACTIVITY_KPI.equals(counter.getType())){
    ...
    private List<FibonacciKPIConfig> getFibonacciKPIsByOrganizationConfig(List<FibonacciKPIConfig> fibonacciKPIConfigs, Long organizationId,ConfLevel level){
        ...
        List<ApplicableKPI> applicableKPIS = new ArrayList<>();
                if(ConfLevel.UNIT.equals(level)){
                    applicableKPIS = counterRepository.getApplicableKPI(new ArrayList(fibonacciKPIConfigMap.keySet()), ConfLevel.UNIT,organizationId);
                }else if(ConfLevel.STAFF.equals(level)){
