...
public class UpdateTimeAndPayoutDetails {
    ...
    public static void updateTimeBankAndPayoutDetails(ShiftImp shiftImp){
        if(isNotNull(shiftImp.getStart())) {
            new CalculatePlannedHoursAndScheduledHours(shiftImp).calculate();
        }
    }
    ...
    public static class CalculatePlannedHoursAndScheduledHours {
        ...
        public CalculatePlannedHoursAndScheduledHours(ShiftImp shift) {
            this.dateTimeInterval = new DateTimeInterval(getStartOfDay(shift.getStart()), getEndOfDay(shift.getStart()));
            this.dayTypeDTOMap = shift.getEmployee().getUnit().getDayTypeMap();
            this.shift = shift;
        }

        public CalculatePlannedHoursAndScheduledHours calculate() {
            boolean ruleTemplateValid = false;
            for (CTARuleTemplate ruleTemplate : shift.getEmployee().getLocalDateCTARuletemplateMap().getOrDefault(shift.getStartDate(),new ArrayList<>())) {
                ...
                if (ruleTemplate.getCalculationFor().equals(FUNCTIONS) && ruleTemplateValid) {
                    BigDecimal value = getFunctionalBonusCompensation(shift.getEmployee().getEmployment(), ruleTemplate, dateTimeInterval);
                    shift.getEmployee().getFunctionalBonus().put(shift.getStartDate(),value);
                }
            }
            return this;
        }
        ...
        public ShiftActivity getShiftActivityDTO(ShiftImp shift, ShiftActivity shiftActivity) {
            try {
